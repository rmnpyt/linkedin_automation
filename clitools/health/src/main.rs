//! Thin CLI wrapper around `health::process` (see `src/lib.rs` for the actual logic and its
//! tests). This binary owns only the stdin/stdout/exit-code contract; it must never grow
//! business logic of its own -- that would duplicate what `MCP/rust-runtime` also depends on.

use std::io::{self, Read, Write};

use health::{process, ToolOutput};
