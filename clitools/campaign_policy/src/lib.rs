...
//! may be -- pure deterministic policy logic, no I/O, following the same contract as
//! `clitools/health` and `clitools/scoring` (PLAN.md sec.5, CONTRACT.md sec.7).
//!
//! This crate is both a library (the `process` function below, reused by `MCP/rust-runtime`
//! so the HTTP transport layer never re-implements this policy logic) and a CLI binary
...
pub const TOOL_NAME: &str = "campaign_policy";
...
pub fn process(raw_input: &str) -> ToolOutput {
    ...
    if trimmed.is_empty() {
        return err("INVALID_INPUT", "input was empty; campaign_policy requires a JSON input object");
    }

    let input: PolicyInput = match serde_json::from_str(trimmed) {
        Ok(parsed) => parsed,
        Err(e) => return err("INVALID_INPUT", format!("input was not a valid policy input: {e}")),
    };
