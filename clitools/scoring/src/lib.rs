...
//! one JSON object in, one JSON object out, no network access, no secrets. It performs only
//! deterministic arithmetic on KPI judgments an LLM has already produced -- it never calls an
//! LLM and never invents a score (CONTRACT.md sec.7,20.1,24).
//!
//! This crate is both a library (the `process` function below, reused by `MCP/rust-runtime`
//! so the HTTP transport layer never re-implements this arithmetic) and a CLI binary
//! (`src/main.rs`) that wraps `process` behind the stdin/stdout/exit-code contract.
//!
...
pub const TOOL_NAME: &str = "scoring";
...
pub struct FilterResults {
    ...
    pub field_passed: Option<bool>,
    pub location_passed: Option<bool>,
...
pub struct ScoringResult {
    ...
    pub effective_scores: BTreeMap<&'static str, Option<i32>>,
    #[serde(rename = "scoreAdjustments")]
    pub score_adjustments: BTreeMap<&'static str, ScoreAdjustment>,
    #[serde(rename = "effectiveWeights", skip_serializing_if = "Option::is_none")]
    pub effective_weights: Option<EffectiveWeights>,
    ...
    pub composite_score_display: Option<f64>,
    #[serde(rename = "qualificationThreshold")]
...
/// the HTTP runtime), decide the outcome.
pub fn process(raw_input: &str) -> ToolOutput {
    let trimmed = raw_input.trim();
    if trimmed.is_empty() {
        return err("INVALID_INPUT", "input was empty; scoring requires a JSON input object");
    }

    let input: ScoringInput = match serde_json::from_str(trimmed) {
        Ok(parsed) => parsed,
        Err(e) => return err("INVALID_INPUT", format!("input was not a valid scoring input: {e}")),
    };
